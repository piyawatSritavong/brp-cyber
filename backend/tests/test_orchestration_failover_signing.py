...
from app.services import control_plane_orchestration_failover_signing as sig
...
def test_signed_orchestration_failover_report_chain() -> None:
    fake = FakeRedis()
    sig.redis_client = fake
    sig.orchestration_failover_enterprise_snapshot = lambda db, limit=200: {"count": 1, "rows": [{"tenant_code": "acb"}]}
    ...
    created = sig.create_signed_orchestration_failover_report(
        db=None,
        destination_dir="./tmp/compliance/test_orchestration_failover",
        limit=10,
    )
    assert created["status"] == "signed"

    status = sig.signed_orchestration_failover_report_status(limit=10)
    assert status["count"] == 1

    verify = sig.verify_signed_orchestration_failover_report_chain(limit=10)
